...
You will need the program Geany installed. If you don't have Geany installed you need to do it click [here](https://download.geany.org/geany-1.36_setup.exe). This will allow you to download the installer, just execute it and follow the instructions to setup Geany.
...
1. Download SFML from the following link
   https://www.sfml-dev.org/download/sfml/2.5.1/
      ...
      - If your MinGW compiler version is the same as one of the SFML versions on the website, download that SFML installer.
      ...
      - If you don't have the correct version (MinGW Builds 7.3.0 (64-bit)) or don't have MinGW compiler then download MinGW Builds 7.3.0 (64-bit), link is in the orange box of the above SFML website or you can just click [here](https://sourceforge.net/projects/mingw-w64/files/Toolchains%20targetting%20Win64/Personal%20Builds/mingw-builds/7.3.0/threads-posix/seh/x86_64-7.3.0-release-posix-seh-rt_v5-rev0.7z/download). You will need to set up MinGW first.
      
**Setting up MinGW Compiler**
  - Extract the MinGW compiler and save it on your computer.
  - If you have trouble extracting it:
       - Download BreeZip from your Microsoft Store.
       - Open the installed MinGW compiler folder in BreeZip and extract it.
   - It's best if you save the folder directly to your computer's C drive which is This PC > Windows (C:)
   - In the MinGW folder, find the bin folder and copy the path of that directory of it from along the top (it should be something like "C:\MinGW\bin")
   - Then launch "Control Panel" go to System and Security --> System --> Advanced system settings --> Environment variables --> Under system variable select Path and click Edit --> New --> paste your copied directory of the path to bin folder in MinGW into it.
   - Check if the MinGW compiler is installed:
       - Open Window Powershell --> type in g++ --> if you see the message "fatal error: no input files" then you are good.
   - Now you should be able to run C++ language in any platform that accommodates it.
 ...
 Option 1:
   ...
   - Downloading this whole repository by clicking the download button.
   - Extract the AVC_Win10.zip folder you download off GitHub.
   - The AVC_Win10 folder is now all set up just like that.
 ...
 Option 2:
   ...
   - With Geany open "robot.cpp" in the "AVC_robot" folder in the "AVC_Win10" folder you just downloaded and delete all the code there so you can paste in the code you just copied.
 ...
 5. Now you need to make the executable files for the robot and server3
 ...
 6. Now you should be able to make the program run.
      ...
      - The program is now running, so switch back over to the window that showed the robot on the green environment and watch it get to the flag!
